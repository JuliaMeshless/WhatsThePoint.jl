...
@testitem "generate_shadows with Constant Δ" setup = [TestData, CommonImports] begin
    ...
    for (i, (sp, exp)) in enumerate(zip(shadow_points, expected))
        sp_coords = to(sp)
        exp_coords = to(exp)
        @test sp_coords[1] ≈ exp_coords[1] atol = 1.0e-10u"m"
        @test sp_coords[2] ≈ exp_coords[2] atol = 1.0e-10u"m"
    end
end
...
@testitem "generate_shadows with Function Δ" setup = [TestData, CommonImports] begin
    ...
    @test sp1_coords[1] ≈ 1.8u"m" atol = 1.0e-10u"m"

    sp2_coords = to(shadow_points[2])
    @test sp2_coords[1] ≈ 3.6u"m" atol = 1.0e-10u"m"
end
...
@testitem "generate_shadows 3D" setup = [TestData, CommonImports] begin
    ...
    sp1_coords = to(shadow_points[1])
    @test sp1_coords[1] ≈ 0.75u"m" atol = 1.0e-10u"m"
    @test sp1_coords[2] ≈ 0.0u"m" atol = 1.0e-10u"m"
    @test sp1_coords[3] ≈ 0.0u"m" atol = 1.0e-10u"m"

    sp2_coords = to(shadow_points[2])
    @test sp2_coords[1] ≈ 0.0u"m" atol = 1.0e-10u"m"
    @test sp2_coords[2] ≈ 0.75u"m" atol = 1.0e-10u"m"
    @test sp2_coords[3] ≈ 0.0u"m" atol = 1.0e-10u"m"
end
